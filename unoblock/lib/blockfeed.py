"""
blockfeed: sync with and process new blocks from unoparty-server
"""
import re
import os
import sys
...
from unoblock.lib import config, util, blockchain, cache, database
from unoblock.lib.processor import MessageProcessor, MempoolMessageProcessor, BlockProcessor, CaughtUpProcessor
...
def fuzzy_is_caught_up():
    """We don't want to give users 525 errors or login errors if unoblockd/unoparty-server is in the process of
    getting caught up, but we DO if unoblockd is either clearly out of date with the blockchain, or reinitializing its database"""
...
def process_cp_blockfeed():
    ...
    config.state['my_latest_block'] = {'block_index': 0}  # last block that was successfully processed by unoblockd
    config.state['last_message_index'] = -1  # initialize (last processed message index)
    config.state['cp_latest_block_index'] = 0  # last block that was successfully processed by unoparty
    config.state['cp_backend_block_index'] = 0  # the latest block height as reported by the cpd blockchain backend
    config.state['cp_caught_up'] = False  # whether unoparty-server is caught up to the backend (e.g. bitcoind)
                ...
                for function in MempoolMessageProcessor.active_functions():
                    ...
                    if result == 'ABORT_THIS_MESSAGE_PROCESSING' or result == 'continue':
                        ...
                        raise Exception(
                            ...
                            (function, result))
        logger.debug("Mempool refresh: {} entries retrieved from unoparty-server, {} new".format(len(new_txs['result']) if new_txs else '??', (len(new_txs['result']) - num_skipped_tx) if new_txs else '??'))
    ...
    if(app_config.count() == 0 or
       ...
       app_config[0]['running_regtest'] != config.REGTEST):
        if app_config.count():
            logger.warn("unoblockd database version UPDATED (from %i to %i) or testnet/regtest setting changed (from %s to %s, or %s to %s). REBUILDING FROM SCRATCH ..." % (
            ...
            logger.warn("unoblockd database app_config collection doesn't exist. BUILDING FROM SCRATCH...")
        ...
        if my_latest_block:
            # remove any data we have for blocks higher than this (would happen if unoblockd or mongo died
    ...
    # avoid contacting unoparty-server (on reparse, to speed up)
    autopilot = False
    autopilot_runner = 0
    iteration = 0
    ...
    # start polling unoparty-server for new blocks
    ...
    while True:
        ...
        if not autopilot or autopilot_runner == 0:
            try:
                ...
                logger.warn("Cannot contact unoparty-server (via get_running_info): {}".format(e))
                time.sleep(3)
                continue

        # wipe our state data if necessary, if unoparty-server has moved on to a new DB version
        wipeState = False
        updatePrefs = False

        # Checking appconfig against old running info (when batch-fetching) is redundant
        if    app_config['unopartyd_db_version_major'] is None \
           or app_config['unopartyd_db_version_minor'] is None \
           or app_config['unopartyd_running_testnet'] is None \
           or app_config['unopartyd_running_regtest'] is None:
            logger.info("Updating version info from unoparty-server")
            updatePrefs = True
        elif cp_running_info['version_major'] != app_config['unopartyd_db_version_major']:
            logger.warn(
                "unoparty-server MAJOR DB version change (we built from %s, unoparty-server is at %s). Wiping our state data." % (
                    app_config['unopartyd_db_version_major'], cp_running_info['version_major']))
            wipeState = True
            updatePrefs = True
        elif cp_running_info['version_minor'] != app_config['unopartyd_db_version_minor']:
            logger.warn(
                "unoparty-server MINOR DB version change (we built from %s.%s, unoparty-server is at %s.%s). Wiping our state data." % (
                    app_config['unopartyd_db_version_major'], app_config['unopartyd_db_version_minor'],
                    cp_running_info['version_major'], cp_running_info['version_minor']))
            wipeState = True
            updatePrefs = True
        elif cp_running_info.get('running_testnet', False) != app_config['unopartyd_running_testnet']:
            logger.warn("unoparty-server testnet setting change (from %s to %s). Wiping our state data." % (
                app_config['unopartyd_running_testnet'], cp_running_info['running_testnet']))
            wipeState = True
            updatePrefs = True
        elif cp_running_info.get('running_regtest', False) != app_config['unopartyd_running_regtest']:
            logger.warn("unoparty-server regtest setting change (from %s to %s). Wiping our state data." % (
                app_config['unopartyd_running_regtest'], cp_running_info['running_regtest']))
            wipeState = True
            updatePrefs = True
        if wipeState:
        ...
        if updatePrefs:
            app_config['unopartyd_db_version_major'] = cp_running_info['version_major']
            app_config['unopartyd_db_version_minor'] = cp_running_info['version_minor']
            app_config['unopartyd_running_testnet'] = cp_running_info['running_testnet']
            app_config['unopartyd_running_regtest'] = cp_running_info['running_regtest']
        ...
        # work up to what block unopartyd is at
        try:
            if cp_running_info['last_block']:  # should normally exist, unless unoparty-server had an error getting it
                ...
                config.state['cp_latest_block_index'] = cp_running_info['unobtanium_block_count']
            else:
                assert False
        except:
            logger.warn(
                "unoparty-server not returning a valid last processed block (probably is reparsing or was just restarted)."
                + " Waiting 3 seconds before trying again... (Data returned: %s, we have: %s)" % (
                    cp_running_info, config.state['cp_latest_block_index']))
            time.sleep(3)
            continue

        config.state['cp_backend_block_index'] = cp_running_info['unobtanium_block_count']
        ...
        if config.state['my_latest_block']['block_index'] < config.state['cp_latest_block_index']:
            ...
            try:
                ...
                # unoparty-server might have gone away...
                my_latest_block = config.mongo_db.processed_blocks.find_one(sort=[("block_index", pymongo.DESCENDING)])
                if my_latest_block:
                    database.rollback(my_latest_block['block_index'])

                # disable autopilot this next iteration to force us to check up against unoparty-server
            ...
            # before what unopartyd is saying if we see this
            logger.error(
                "Very odd: Ahead of unoparty-server with block indexes! Pruning back %s blocks to be safe."
            ...
            # ...we may be caught up (to unopartyd), but unopartyd may not be (to the blockchain). And if it isn't, we aren't
            config.state['caught_up'] = config.state['cp_caught_up']

            # this logic here will cover a case where we shut down unoblockd, then start it up again quickly...
            ...
            publish_mempool_tx()
            time.sleep(2)  # unoblockd itself is at least caught up, wait a bit to query again for the latest block from cpd
