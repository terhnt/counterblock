...
from unoblock.lib import config, config_util, log, blockfeed, util, module, database
from unoblock.lib.processor import messages, caughtup, startup  # to kick off processors
from unoblock.lib.processor import StartUpProcessor
...
CONFIG_ARGS = [
    ...
    [('--unoparty-connect',), {'help': 'the hostname of the unopartyd JSON-RPC server'}],
    [('--unoparty-port',), {'type': int, 'help': 'the port used to communicate with unopartyd over JSON-RPC'}],
    [('--unoparty-user',), {'help': 'the username used to communicate with unopartyd over JSON-RPC'}],
    [('--unoparty-password',), {'help': 'the password used to communicate with unopartyd over JSON-RPC'}],
    ...
    [('--rpc-port',), {'type': int, 'help': 'port on which to provide the unoblockd JSON-RPC API'}],
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='unoblock', description='unoblock daemon')

    parser.add_argument(
        '-V', '--version', action='version', version="unoblock %s" % config.VERSION)
    ...
    parser_server = subparsers.add_parser('server', help='Run unoblock')
    parser_reparse = subparsers.add_parser('reparse', help='Reparse the unoblock database')
    parser_enmod = subparsers.add_parser('enmod', help='Enable a module')
    parser_enmod.add_argument('module_path', type=str, help='Full Path of module to Enable relative to Unoblockd directory')
    parser_dismod = subparsers.add_parser('dismod', help='Disable a module')
    parser_dismod.add_argument('module_path', type=str, help='Path of module to Disable relative to Unoblockd directory')
    ...
    # Handle arguments
    logger.info("unoblock command specified: {}".format(args.command))
    ...
    logger.info("unoblock Version %s starting ..." % config.VERSION)
